...
% Last Modified by GUIDE v2.5 10-Oct-2015 15:40:26
...
% varargin   command line arguments to kras_lab1_main2 (see VARARGIN)

% Choose default command line output for kras_lab1_main2
...
% subs(subs(dfdp1,p1,xp1(2)),p2,xp2(2))
% subs(subs(dfdp2,p1,xp1(2)),p2,xp2(2))
...
CR=0.2;
plotCircle(0,0, CR);xlabel('x1,p1');ylabel('x2,p2');
...
axes(handles.axes6)
plot(a1);grid on;xlabel({'Коэф. пропорциональности ','для карты (U_{1},\phi_{1})'});

axes(handles.axes7)
plot(a2);grid on;xlabel({'Коэф. пропорциональности ','для карты (U_{2},\phi_{2})'});
